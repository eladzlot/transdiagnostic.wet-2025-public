...
On the cognitive subscale, WET scored `r format_mean_sd(cognitive_score_wet)`, whereas NIW scored `r format_mean_sd(cognitive_score_ew)`.
...
This style of writing resembles techniques used in imagery rehearsal interventions [cf. @krakowClinicalManagementChronic2006].
...
```{r engagement, echo=FALSE, include=FALSE, cache=TRUE}
# Source the growth model script
source(here('scripts', 'model.growth.R'))

raw.engagement = read_csv(here('raw/engagement.csv'))

# Filter and join the data for engagement measure
d <- data_long %>%
  filter(measure == 'oasis', condition != 'waitlist', !is.na(value), wave > 0) %>%
  left_join(demographics.data, by = c("id", 'condition')) %>%  # Ensure proper join key if needed
  left_join(raw.engagement, by = 'id') %>%
  # We define niw as waitlist and engagement as treatment/control for convinience
  mutate(condition = case_when(
    condition == 'control' ~ 'waitlist',
    engagement.1 == 1 ~ 'treatment',
    TRUE ~ 'control'
  ))

n_engaged = sum(d$engagement.1)

sd.engagement.yes = d %>% filter(wave == 1, condition == 'treatment') %>% group_by(id) %>% summarize(v = sum(value)) %>% pull(v) %>% sd
sd.engagement.no = d %>% filter(wave == 1, condition == 'control') %>% group_by(id) %>% summarize(v = sum(value)) %>% pull(v) %>% sd
sd.engagement.niw = d %>% filter(wave == 1, condition == 'waitlist') %>% group_by(id) %>% summarize(v = sum(value)) %>% pull(v) %>% sd

# Fit the growth model
m <- model.growth(d, 'enagement', force = FALSE)
...
# Calculate deltas for WET, NIW, and WL conditions
delta.treatment.yes <- (expected[[1]][, 1] - expected[[1]][, 4])/sd.engagement.yes
delta.treatment.no <- (expected[[2]][, 1] - expected[[2]][, 4])/sd.engagement.no
delta.treatment.niw <- (expected[[3]][, 1] - expected[[3]][, 4])/sd.engagement.niw
delta.fu1.yes <- (expected[[1]][, 1] - expected[[1]][, 5])/sd.engagement.yes
delta.fu1.no <- (expected[[2]][, 1] - expected[[2]][, 5])/sd.engagement.no
delta.fu1.niw <- (expected[[3]][, 1] - expected[[3]][, 5])/sd.engagement.niw
delta.fu2.yes <- (expected[[1]][, 1] - expected[[1]][, 6])/sd.engagement.yes
delta.fu2.no <- (expected[[2]][, 1] - expected[[2]][, 6])/sd.engagement.no
delta.fu2.niw <- (expected[[3]][, 1] - expected[[3]][, 6])/sd.engagement.niw
```

Most `r format_count_perc(n_engaged, nrow(d))` of the participants who completed WET were classified as having engaged with their identified core threat at least once across the four writing sessions.
Across all stages, participants who engaged showed better outcomes than those who did not.

During treatment, engaged participants showed a small advantage over non-engaged participants (`r format_estimate(delta.treatment.yes - delta.treatment.no)`).
At the one-week follow-up the difference was larger and statistically reliable (`r format_estimate(delta.fu1.yes - delta.fu1.no)`), whereas at the two-month follow-up the estimated difference was smaller and less certain (`r format_estimate(delta.fu2.yes - delta.fu2.no)`).

When compared with the NIW group, outcomes during treatment were similar (`r format_estimate(delta.treatment.yes - delta.treatment.niw)`).
A modest advanage was observed for engagers at the one-week follow-up (`r format_estimate(delta.fu1.yes - delta.fu1.niw)`), and by the two-month follow-up the difference was larger and statistically reliable (`r format_estimate(delta.fu2.yes - delta.fu2.niw)`).
...
For metacognitions measured by the MCQ, both maladaptive indices-(lack of) cognitive confidence (`r format_estimate(d.mcq_confidence)`) and negative beliefs about uncontrollability and danger (`r format_estimate(d.mcq_danger)`) increased significantly in WET.
...
The only significant between group difference was found in (lack of) cognitive confidence (`r format_estimate(d.mcq_confidence.ew - d.mcq_confidence)`).
...
This between-session reduction in state anxiety predicted improvements in trait anxiety (OASIS), in line with meta-analysis showing that between session fear reduction is correlated with treatment outcome [e.g., @ruppEmotionalProcessingTheory2017].
...
In the present study, most participants reported that the instructions were clear; a minority indicated difficulty understanding the instructions to return to the same script, which will be clarified in future versions.
