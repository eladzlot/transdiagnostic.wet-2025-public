---
title           : "Transdiagnostic Written Exposure Therapy: a Randomized Control Trial"
...
The intervention will be tested in an online randomized controlled trial with a trans-diagnostic sample of individuals with high levels of anxiety.
Therefore, we will test treatment efficacy as measured by change in anxiety and functioning.
Furthermore we will investigate patterns of change and potential mechanisms of change.
...
1. WET is more effective than control in reducing pathological anxiety.
2. Changes in proposed mechanisms change together with changes in pathological anxiety. This could indicate the possibility that the mechanism serves as a mediator, and provide motivation for further studies.
...
* Participants will be randomly assigned to one of two groups: the treatment group (WET) and the control group (expressive writing).
* Both groups will engage in four sessions of writing and a follow-up session.
* Symptom measures and mechanisms will be assessed before the first and after the last writing sessions.
* Symptoms will be measured again at the follow-up session.
* State anxiety and thought action fusion will be measured at each writing session.

Treatment efficacy will be investigated within a pretest-posttest control group design.
...
Participants will be randomly assigned to the treatment and control groups in a 2:1 ratio, with the treatment group being twice the size of the control group.
...
Participants will be paid 9 GBP/hour for agreeing to participate.
...
Exclusion criteria are severe depression [greater than 14 on the PHQ; @kroenkePHQ8MeasureCurrent2009], post-trauma [greater than 6 on the PCL-5; @blevinsPosttraumaticStressDisorder2015], or psychotic symptoms [items 19 or 20 from the DIAMOND screener; @tolinPsychometricPropertiesStructured2018].
In addition, English as a fluent language is required.
Furthermore, participants who have reported head Injury or reading and writing difficulties in the Prolific system  will be excluded.
...
We aim to recruit a minimum of 60 participants and possibly up to 150 participants in increments of 30.
The recruitment process will continue until we reach the maximum participant count 
or until the difference between groups on the OASIS reaches a region of practical equivalence (ROPE) of 89% for a two-point effect size (see model below).
...
A power analysis was conducted with the assumption of an intraclass correlation coefficient (ICC) of 0.5 and an effect size of 0.4, revealing that a sample size of 148 participants would provide 0.8 power.
However, in consideration of financial constraints, a Bayesian stopping criteria was implemented to enable early termination of the study.
...
See above.
Additionally we will recruit until we reach our sample size, or after 6 months from the beginning of the recruitment. 
...
The treatment group receive instructions to write about their core threats, whereas the control group receive instructions to write about a daily activity.
...
1. Thought Fusion Inventory [TFI; @wellsThoughtFusionInstrument2001]
2. Metacognition Questionnaire [MCQ; @wellsShortFormMetacognitions2004]
3. Distress Tolerance Scale [DTS; @simonsDistressToleranceScale2005]
4. Impact of Future Events Scale [IFES; @deeproseMeasuringIntrusiveProspective2011]
5. Brief Core Schema Scale [BCSS; @fowlerBriefCoreSchema2006]
6. Imaginal Behavior Approach Test (iBAT; Barzilai & Huppert, manuscript in preparation)
7. Self compassion scale - short form [SCS-SF; @babenkoMeasuringSelfCompassionMedical2019]
...
\phi_{itq} = \alpha_q \cdot (\theta_{itq} - \beta_q) \tag{IRT model}  \\
\theta_{it} = \gamma_{0i} + t \cdot \tau_i  \tag{trait model} \\
\tau_i = \delta_{1i} \cdot C_i \tag{change model} \\
\begin{bmatrix} \gamma_{0i} \\ \delta_{1i} \end{bmatrix} \sim \text{MVNormal} \left( \begin{bmatrix} \bar{\theta} \\ \bar{\tau} \end{bmatrix}, \rho \right) \tag{person effects} \\
\kappa \sim \text{Normal}(0,1.5) \tag{cutpoints} \\
\bar{\theta} \sim \text{Normal}(0,0.3) \tag{latent baseline trait} \\
\bar{\tau} \sim \text{Normal}(0,0.3) \tag{treatment effect on trait} \\
\beta_q \sim \text{Normal}(0,0.3) \tag{IRT location parameter} \\
\alpha_q \sim \text{Log-Normal}(0,1) \tag{IRT discrimination parameter} \\
\rho \sim \text{LKJcorr}(2) \tag{correlation matrix}
\end{align*}

Where $R_{itq}$ is the response to question $q$ at time $t$ by participant $i$.
And $C_i$ is the condition of participant $i$ (condition will be contrast encoded).

To this base model we will attempt to add gender as a predictor of both the latent trait $\theta$ and latent change $\tau$.
We will add them each separately, and evaluate them via WAIC [@watanabeAsymptoticEquivalenceBayes2010], where the criterion for inclusion is an improvement of at least 10 points in the WAIC.
...
The estimand of interest is the treatment effect $\tau$, though we will simulate outcomes to interpret the results.
We will run the base model for each of the symptoms and mechanisms measured.
...
The only difference being that the trait ($\theta$) and change ($\tau$) parameters of both models will be sampled from the same multi-variate normal distribution.
...
Time will be zero base encoded (pre-treatment will be coded 0).
Category and gender will be contrast encoded.
...
We will assume that missining may be affected both by baseline trait levels and by difficulty completing the task
